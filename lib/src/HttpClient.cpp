...
namespace s3benchmark {
    ...
    HttpClient::HttpClient(const std::string &initial_request_headers,
            ...
            , request_headers(new char[request_header_size + sizeof(HEADER_PADDING)])
        ...
        for (int i = 0; i < sizeof(HEADER_PADDING); ++i) {
            this->request_headers[request_header_size + i] = HEADER_PADDING[i];
        }
    }
       ...
       //  while(true) {
        ...
        if (poll_res == 0) {
            std::cerr << "Timeout while polling on socket " << this->socket_descriptor
                      << " with events 0x" << std::hex << poll_def.revents << std::dec
                      << std::endl;
            return;
        } else if (poll_res < 0) {
        ...
        if ((poll_def.revents & POLLNVAL) || (poll_def.revents & POLLERR)) {
        ...
        if (poll_def.revents & POLLIN) {
            ...
            int recv_len, recv_sum = 0;
            do {
                recv_len = recv(this->socket_descriptor, recv_buffer, buffer_size, 0);
                if (recv_len == -1) {
                    auto read_err = errno;
                    // if (read_err == EWOULDBLOCK ) { //&& !((poll_def.revents & POLLHUP))) {
                    //     goto poll;
                    // }
